...
elseif(strcmp(deconv_method,'grl') || strcmp(deconv_method,'mfod'))
    ...
    for rf_id=1:length(aniso_rf)
        ...
        if(contains(requested_rf{1},'dki') > 0 && ischar(eigvals))
            if(contains(eigvals,'auto'))
            ...
            elseif(contains(requested_rf{1},'noddi'))
                SD.AddAnisotropicRF_NODDI(requested_rf{2});
            end
        end
    end
    
    for rf_id=1:length(iso_rf)
        SD.AddIsotropicRF(iso_rf{rf_id}*10e-3);  
    end

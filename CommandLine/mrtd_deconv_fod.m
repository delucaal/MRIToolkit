function mrtd_deconv_fod(varargin)
    ...
    if(isempty(coptions) || isempty(coptions{1}) || strcmpi(coptions{1},'-help'))
        ...
        help = [help newline '-method: one in "csd","mscsd","grl","grl_auto","mfod","lasso"]'];
        ...
        help = [help newline '-reg_factor: regularization factor (mFOD and LASSO only, default is 0.01)'];
        help = [help newline];
...
for input_id =1:2:length(coptions)
    ...
    elseif(strcmp(value,'-reg_factor'))
        reg_factor = coptions{input_id+1};
    end
...
if(isempty(mat_file))
    ...
    temp_mat_file = MRTQuant.QuickNiiBvalBvecToMat('nii_file',nii_file,...
...
elseif(strcmp(deconv_method,'grl') || strcmp(deconv_method,'mfod') || strcmp(deconv_method,'lasso'))
    ...
    elseif(strcmp(deconv_method,'mfod'))
        ...
        SD.setL2REG(reg_factor);
        SD.setDeconvMethod('L2LSQ');
    elseif(strcmp(deconv_method,'lasso'))
        SD.setInnerShellWeighting(1.0); % in mFOD this parameter has not been investigated
        SD.setL2REG(reg_factor);
        SD.setDeconvMethod('LASSO');
    end

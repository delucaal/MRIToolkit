...
if(isempty(coptions) || isempty(coptions{1}) || strcmpi(coptions{1},'-help'))
    ...
    help = [help newline];
    help = [help newline '-grad_header(beta): 0 off (default) use grad_perm/flip as below, or 1 permute/flip the gradients according to the header.'];
    ...
    help = [help newline '-preserve_header: 0 (default) use a normalized ExploreDTI-like header, or 1 to preserve the original reference to world coordinates.'];
    help = [help newline '                  If you plan on using ExploreDTI in subsequent steps, please use the normalized header.'];
    help = [help newline];
    fprintf(help);
...
for input_id =1:2:length(coptions)
    ...
    elseif(strcmp(value,'-preserve_header'))
        preserve_header = str2double(coptions{input_id+1});
    elseif(strcmp(value,'-grad_header'))
        grad_header = str2double(coptions{input_id+1});
    elseif(strcmp(value,'-grad_conform'))
        grad_conform = str2double(coptions{input_id+1});
    end
...
MRTQuant.PerformDTI_DKIFit('nii_file',temp_file,'txt_file',[dest_basename '.txt'],...
    ...
    'mk_curve',mkcurve,'fit_mode',estimator,'grad_header',grad_header,'grad_conform',grad_conform);

MRTQuant.MatMetrics2Nii([outfile '.mat'],dki,preserve_header);

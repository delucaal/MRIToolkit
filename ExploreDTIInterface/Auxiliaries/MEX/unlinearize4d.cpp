...
template<class T> inline void function(int nlhs, mxArray* plhs[], int nrhs, const mxArray *prhs[]) {
    const mwSize* lin_size = mxGetDimensions(prhs[0]);
    const mwSize* mask_size = mxGetDimensions(prhs[1]);
    T* lin = (T*) mxGetData(prhs[0]);
    ...
    const mwSize unlin_size[] = {mask_size[0],mask_size[1],mask_size[2],lin_size[0]};
    
    plhs[0] = (mxArray*)mxCreateNumericArray(4,unlin_size,mxGetClassID(prhs[0]),mxREAL);
    T* unlin = (T*) mxGetData(plhs[0]);
            ...
            for (int y = 0; y < unlin_size[1]; y++) {
                for (int x = 0; x < unlin_size[0]; x++) {
                    if (mask[offset3(x,y,z,mask_size[0],mask_size[1])]) {
                        unlin[offset4(x,y,z,w,unlin_size[0],unlin_size[1],unlin_size[2])] = lin[offset2(w,count,lin_size[0])];
                        count++;
                    } else {
                        unlin[offset4(x,y,z,w,unlin_size[0],unlin_size[1],unlin_size[2])] = std::numeric_limits<T>::quiet_NaN();
                    }
                }
            }
        }

...
template<class T> inline void function(int nlhs, mxArray* plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    for (int pos = 0; pos < points_size[1]; pos++) {
        ...
        T p0 = point[0] / vdims[0];
        T p1 = point[1] / vdims[1];
        T p2 = point[2] / vdims[2];
        
        bool oob = false;
        
        int min0 = int(p0); if (min0 < 1) oob = true;
        int min1 = int(p1); if (min1 < 1) oob = true;
        int min2 = int(p2); if (min2 < 1) oob = true;
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray *prhs[]) {
    if ((mxGetClassID(prhs[0]) != mxGetClassID(prhs[1])) || (mxGetClassID(prhs[0]) != mxGetClassID(prhs[2]))) {

...
%                     [~,IX] = max(abs(smat(ij,:)));
            ...
            json.Description = my_help('MRTQuant.ConformSpatialDimensions');
            json.ReferenceFile = nii_file;
            json.ProcessingType = 'Preprocessing';
        ...
        function RestoreSpatialDimensions(varargin)
            % Restores the original coordinate system from a json definition file. Input arguments:
            % nii_file: the input file
            % output: the output file
            % json_file: describing the reference coordinate system. 
            if(isempty(varargin))
                my_help('MRTQuant.RestoreSpatialDimensions');
                return;
            end
            ...
            json_in = GiveValueForName(coptions,'json_file');
            if(isempty(json_in))
                error('Missing mandatory parameter json_file');
            end

            data_in = nii_file;
            data_out = output;
            json_def = NiftiIO_basic.ReadJSONDescription('json_file',json_in);
